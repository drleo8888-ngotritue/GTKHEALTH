...
export const Kiosk: React.FC<{ stationId: string; stationName: string }> = ({ stationId, stationName }) => {
  ...
  const processSubmit = (idToCheck: string) => {
    ...
    if (dbPatient) {
        setPatientInfo({ name: dbPatient.name, dept: dbPatient.department });
        setStep(2);
    } else {
        setStep('error');
    }
    ...
    <div className="w-full h-screen p-4 flex items-center justify-center bg-gray-100 select-none">
      ...
      {step === 'error' && (
          <div className="bg-white p-20 rounded-[3rem] shadow-2xl text-center relative overflow-hidden max-w-4xl w-full border border-red-200">
            ...
            <h2 className="text-5xl font-extrabold text-gray-800 mb-6 tracking-tight">Mã thẻ chưa có trong hệ thống</h2>
            <p className="text-3xl text-gray-600 font-medium leading-snug">Mã thẻ của bạn hiện chưa có trong cơ sở dữ liệu,<br/>vui lòng đưa thẻ cho Nhân viên Y tế để tiếp tục.<br/><span className="text-medical-green">Xin cảm ơn!</span></p>
            <p className="text-2xl text-gray-400 mt-4 mb-12">您的卡号不在数据库中，请将卡交给医务人员继续。谢谢！</p>
