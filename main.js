...
async function doServerSync() {
  ...
  try {
    ...
    // SPOKE: chỉ TIẾP NHẬN phiếu (đánh dấu đang vận chuyển) — KHÔNG tự cộng kho.
    // Kho chỉ tăng khi nhân viên bấm "đã nhận đủ" (IPC receive-transfer). Đúng thực tế vật lý.
    if (_stationType !== 'HUB') {
      ...
      if (Array.isArray(pendingTransfers) && pendingTransfers.length > 0) {
        ...
        for (const transfer of pendingTransfers) {
          if (transfer.status === 'PENDING') {
            try { await syncServer.ackTransfer(transfer.id); acked++; } catch (_) {}
          }
        }
        // Báo renderer có phiếu đến đang chờ nhận (để chuông noti hiển thị)
        BrowserWindow.getAllWindows().forEach(win => {
          if (!win.isDestroyed()) win.webContents.send('transfer:incoming', { count: pendingTransfers.length });
        });
        if (acked > 0) console.log(`📦 Đã tiếp nhận ${acked} phiếu điều chuyển (chờ nhân viên nhận hàng).`);
      }
    }
...
// Spoke: danh sách phiếu điều chuyển ĐẾN chưa nhận xong (PENDING + ACKNOWLEDGED)
ipcMain.handle('server-sync:list-incoming-transfers', async () => {
  try {
    const data = await syncServer.pullPendingTransfers(syncServer.getStationName());
    return { success: true, data: data || [] };
  } catch (err) {
    return { success: false, data: [], message: err?.message };
  }
...
ipcMain.handle('server-sync:receive-transfer', async (_e, { transfer, receivedItems, alreadyImported, actorName, actorRole } = {}) => {
  try {
    ...
    return { success: false, message: err?.message || 'Lỗi xác nhận nhận thuốc' };
  }
});
